...
const reserved = () => {
  ...
  useEffect(() => {
    const fetchTheaterAndReservedBookings = async () => {
      ...
      const bookings = await client.fetch(
        `*[_type == "booking" && ticketStatus == "reserved" && theaterName == $name] | order(createdAt desc)[0...500]`,
    ...
    fetchTheaterAndReservedBookings();
  }, []);
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-2xl font-semibold">Reserved Bookings</h1>
              ...
              paginatedData.map((b) => (
                <TableRow key={b._id}>
                  ...
                  <TableCell className="capitalize">
                    <div className="w-fit h-fit py-[1px] px-2 rounded-md border border-blue-600 bg-[#F4F6F5] text-blue-600">
                      {b.ticketStatus}
                    </div>
                  </TableCell>
